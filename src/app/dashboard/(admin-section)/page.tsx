...
export default function DashboardPage() {
  ...
  const [onTimeDeliveryPercent, setOnTimeDeliveryPercent] = useState(0)
  ...
  // Calculate warehouse capacity percentage
  const calculateWarehouseCapacity = () => {
    const maxCapacity = 10000 // Set your max capacity here
  ...
  const getRecentActivity = async () => {
    try {
      ...
      inventorySnap.docs.slice(0, 1).forEach(doc => {
        const data = doc.data()
  ...
  // Calculate changes whenever values update
  ...
  const lowStockChange = calculateChangePercent(prevLowStockCount, lowStockCount) // inverted for low stock
  const completedTodayChange = calculateChangePercent(completedTodayCount, prevCompletedToday)

  // Update previous values periodically (e.g., every refresh)
  useEffect(() => {
